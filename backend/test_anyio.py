...
params = StdioServerParameters(command="npx", args=["-y", "@modelcontextprotocol/server-mongodb"], env=os.environ.copy())
    ...
    try:
        async with stdio_client(params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                print("Exp A: Initialized successfully.")
        print("Exp A: Exited successfully.")
    except Exception as e:
        print(f"Exp A Failed: {type(e).__name__}: {e}")
...
async def exp_B():
    ...
    try:
        async with stdio_client(params) as (read, write):
            async with ClientSession(read, write) as session:
                await session.initialize()
                print("Exp B: Initialized successfully. Waiting 2 seconds...")
                await asyncio.sleep(2)
                print("Exp B: Done waiting.")
        print("Exp B: Exited successfully.")
    except Exception as e:
        print(f"Exp B Failed: {type(e).__name__}: {e}")
            ...
            try:
                async with stdio_client(params) as (read, write):
                    async with ClientSession(read, write) as session:
                        ...
                        print("Exp D: Worker received stop event.")
            except Exception as e:
                print(f"Exp D Worker Failed: {type(e).__name__}: {e}")
...
async def main():
    print("--- Starting AnyIO Isolation Experiments ---")
    await exp_A()
    await exp_B()
    await exp_C()
    await exp_D()
